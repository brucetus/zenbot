  ...
  calculate: function (s) {
    if (s.lookback[s.options.min_periods]) {
      if (s.options.buy !== false) {
        if ((s.period.high / s.upfractal > s.options.up) && (s.period.high / Math.max(s.lookback[29].senkou_a, s.lookback[29].senkou_b) > s.options.up) && (s.period.tenkan > s.period.kijun) && (s.period.close > s.lookback[29].close) && (s.period.senkou_a > s.period.senkou_b)) {
          if (s.trend !== 'up') {
      ...
      if (s.options.sell !== false) {
        if ((s.period.low / s.downfractal < s.options.down) && (s.period.low / Math.min(s.lookback[29].senkou_a, s.lookback[29].senkou_b) < s.options.down) && (s.period.tenkan < s.period.kijun) && (s.period.close < s.lookback[29].close) && (s.period.senkou_a < s.period.senkou_b)) {
          if (s.trend !== 'down') {
            s.acted_on_trend = false
          }
          s.trend = 'down'
