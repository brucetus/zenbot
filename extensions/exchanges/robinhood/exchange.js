...
module.exports = function container (conf) {
  ...
  function publicClient() {
    if (!public_client) public_client = new RobinClient()
    return public_client
  }

  function authedClient() {
    if (!authed_client) {
      if (!conf.robinhood || !conf.robinhood.key || conf.robinhood.key === 'YOUR-API-KEY') {
        throw new Error('please configure your Kraken credentials in conf.js')
      }
      authed_client = new RobinClient(conf.robinhood.key, conf.robinhood.secret)
    }
